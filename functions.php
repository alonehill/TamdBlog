...
function themeInit($archive) 
{
    ...
    if ($user->hasLogin() && $user->pass('administrator', true)) {
        ...
        if (isset($_GET['action']) && $_GET['action'] == 'manage_links') {
            ...
            ?>
            <?php include __DIR__ . '/adhtml/FriendLinks_Manage.php'; ?>
            <?php
            exit;
        }
    }
...
function themeConfig($form) { 
    echo '<h2 class="tamd-h2">TamdBlog主题设置</h2>';
    ...
    $navStyle = new Typecho_Widget_Helper_Form_Element_Radio(
        'navStyle',
        array(
            'nav-right' => '<img src="' . $themeUrl . '/adstatic/img/menu-nav-right.png">', 
            'nav-left'  => '<img src="' . $themeUrl . '/adstatic/img/menu-nav-left.png">'
        ),
        'nav-right',
        _t('导航栏布局样式'),
    ...
    include __DIR__ . '/adhtml/ConfigTheme/slideDate_Add.php';
    ...
    include __DIR__ . '/adhtml/ThemeConfig_Style.php';
}
...
/**
 * 为 Typecho 后台编辑器添加自定义样式按钮（稳定版）
 */
function theme_add_editor_style_button() {
    // 仅当有编辑权限时输出
    ...
    (function() {
        // 定义按钮菜单项（按需修改）
        function addStyleButton(editor) {
            editor.addButton('custom_styles', {
                text: '样式',
                type: 'menubutton',
                menu: [
                    {
                        text: '信息提示框',
                        onclick: function() {
                            editor.insertContent('<div class="tip tip-info">请输入内容</div>');
                        }
                    },
                    {
                        text: '警告提示框',
                        onclick: function() {
                            editor.insertContent('<div class="tip tip-warning">请输入内容</div>');
                        }
                    },
                    {
                        text: '下载按钮',
                        onclick: function() {
                            editor.insertContent('<a class="btn-download" href="#">下载链接</a>');
                        }
                    }
                ]
            });
        ...
        // 使用 addeditor 事件，当每一个编辑器初始化时添加按钮
        if (typeof tinymce !== 'undefined') {
            tinymce.on('addeditor', function(e) {
                addStyleButton(e.editor);
            });
        } else {
            // 以防 tinymce 尚未加载（理论上不会）
