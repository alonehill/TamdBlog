...
function themeConfig($form) {
    ...
    if ($updateInfo && version_compare(THEME_VERSION, $updateInfo['version'], '<')) {
        $update_html = <<<HTML
        <div style="color: #467B96; background: #E8F6FF; padding: 10px; border-radius: 4px; margin-bottom: 15px;">
            <strong>发现新版本：{$updateInfo['version']} </strong><br>
            更新日志：<br>{$updateInfo['changelog']}<br><br>
            
            <!-- 注意：将 type 改为 button，避免触发表单默认提交 -->
            <button type="button" class="btn primary" id="btn-theme-update" onclick="startThemeUpdate('{$updateInfo['download_url']}')">一键自动更新</button>
            
            <script>
            function startThemeUpdate(downloadUrl) {
                if (confirm('强烈建议更新前备份当前主题！确认要执行自动更新吗？')) {
                    
                    // 改善体验：点击后禁用按钮并提示更新中
                    var btn = document.getElementById('btn-theme-update');
                    btn.innerText = '正在执行更新，请稍候...';
                    btn.style.opacity = '0.7';
                    btn.style.pointerEvents = 'none';

                    // 动态创建表单并挂载到 body 最后，跳出大表单的嵌套
                    var tempForm = document.createElement('form');
                    tempForm.action = '?theme_action=update_theme';
...
HTML;
        ...
        <div class="theme-update-alert-minimal">
            ...
            <span class="version-badge">v{$theme_wersion}</span>
        </div>
HTML;
    }
    
    $update_ui = new Typecho_Widget_Helper_Layout();
