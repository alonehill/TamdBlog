<?php if (!defined('__TYPECHO_ROOT_DIR__')) exit; ?>
    ...
    <aside id="secondary" class="widget-area" role="complementary">
        ...
        <!-- 名片 -->
        <?php if (!empty($this->options->sidebarBlock) && in_array('ShowName', $this->options->sidebarBlock)): ?>
        <section class="widget widget-profile modern-sidebar-widget">
            <div class="profile-avatar">
                <img class="avatar" src="<?php echo getCustomAvatar($this->author->mail, 60); ?>" alt="" width="60" height="60"/>
            </div>
            <div class="profile-info">
                <h3 class="profile-name"><?php $this->author(); ?></h3>
                <p class="profile-bio"><?php $this->author->url(); ?></p>
            </div>
        </section>
        <?php endif; ?>
        ...
        <section class="widget modern-sidebar-widget">
            ...
            <ul class="widget-list sidebar-post-list-wrapper">
                <?php $this->widget('Widget_Contents_Post_Recent')->to($recent); ?>
                    <?php while($recent->next()): ?>
                    <li class="sidebar-post-item">
                        <a class="sidebar-post-link" href="<?php $recent->permalink(); ?>">
                            <span class="sidebar-post-title"><?php $recent->title(); ?></span>
                        </a>
                    </li>
                <?php endwhile; ?>
            </ul>
                ...
                <?php while ($comments->next()): ?>
                    <li class="comment-item">
                        ...
                        if ($row) {
                            ...
                            preg_match_all('/<img[^>]+class="emoji-img"[^>]*>/', $content, $matches);
                            $emojis = $matches[0];
                            $content = preg_replace('/<img[^>]+class="emoji-img"[^>]*>/', '〓', $content);
                            $content = strip_tags($content);
                            $summary = mb_substr($content, 0, 25, 'UTF-8');
                            $i = 0;
                            $summary = preg_replace_callback('/〓/', function() use (&$i, $emojis) {
                                return isset($emojis[$i]) ? $emojis[$i++] : '〓';
                            }, $summary);
